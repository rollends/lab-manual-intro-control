...
\end{center}
%
As in Lab~\ref{Lab:1}, you should observe that the new time constant \(\tau_2\) of the transfer function from \(v_r(t)\) to \(v(t),\) which we denote \(G_2(s),\) is given by
\[
...
\[
  G_2(s)
    = \frac{V(s)}{V_r(s)}
    = \frac{K_3 K_2}{1 + K_2} \frac{1}{\tau_2 s + 1}.
\]
...
\]
Verify that the transfer function from \(r(t)\) to \(x(t),\) denoted \(G_3(s)\) in the Simulink model, is
\[
