...
output \(Y(s) = G(s)U(s)\) can be written as
\[
  y(t) = \left\|G(j\omega)\right\| \cos\left(\omega t + \measuredangle G(j\omega)\right)
    +
    d(t)
\]
where \(d(t)\) is some stable signal.
It follows that the output \(y(t)\) converges towards the steady-state signal
\[
  y_\mathrm{ss}(t)
    = \left\| G(j\omega) \right\| \cos(\omega t + \measuredangle G(j\omega)).
\]
Recognizing that input was \(u(t) = \cos(\omega t),\) we observe that the
output \emph{in steady state} is an amplified/attenuated and phase shifted
