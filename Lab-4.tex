...
\[
  P(s) = \frac{1}{s^2}.
\]
Take \texttt{ECE 486}\footnote{In the robotics course you will learn about a technique called Feedback Linearization. You just cancel the nonlinearities out!} if you are interested in how to do this.
%
\begin{figure}
  \centering
...
\begin{enumerate}[label=(\arabic*)]
  \item{
    \textbf{Learn} how the derivative and integral terms of a PID controller affect performance.
  }
  \item{
...
We would like to eliminate the constant steady-state error.
How do we do this?
First we must determine why this error appears.
You may have the question ``If our plant already has an integrator, how is it that there is constant steady-state error?''
In this simulation there is a constant \emph{input} disturbance similar to the \(d(t)\) of Lab~\ref{Lab:3}.
As you verified in Lab~\ref{Lab:3}, even an integrator in the plant cannot perfectly reject this error and can only attenuate it under proportional error feedback.
If we introduce a pole at the origin, i.e. an integrator, the controller can perfectly reject this error.
This is the virtue of using the integrator term of PID control.
...
The poles of this system\footnote{Read very carefully here. I am \textbf{not} saying the zeros or the DC gain are the same. Just the poles!} are the same as the poles of the closed loop system
%
\begin{center}
  \begin{tikzpicture}[x=1in, y=1in]
...
\begin{procedure}[label={proc:lab4:ki}]
  ...
  \begin{enumerate}[label={(\arabic*)}]
    \item{%
      \textbf{Run} the ``part\_2.m'' MATLAB script and \textbf{inspect} Figure 3.
    }
    \item{%
...
\begin{deliverable}[label={lab4:report}]
  \begin{enumerate}[label={(\arabic*)}]
    \item{%
      ...
      Unlike Lab~\ref{Lab:3}, we do not observe the velocity explicitly but, instead, use the differentiator in the PID controller (derivative of the relative position is velocity) to estimate velocity and apply that in closed loop feedback.
      The explicit use of a differentiator introduced zeros in the final transfer function \(G(s)\) from \(r(t)\) to \(y(t).\)
      In particular
      \[
        G(s) = K_d \frac{s^2 + \frac{K_p}{K_d} s + \frac{K_i}{K_d}}{s^3 + K_d s^2 + K_p s + K_i}.
      \]
      For the \emph{final value} of \(K_p,\) \(K_i\) and \(K_d\) you chose, \textbf{determine} where the zeros of \(G(s)\) are located.
      \label{lab4:report:q1}
    }
    \item{%
      As discussed briefly in the lab, if the zeros are sufficiently fast (i.e. large negative real part) compared to the poles, we can approximate the behaviour of the system as simply the system without the zeros.
    ...
    \item{%
      Using the statement you proved in~\ref{lab4:report:q3}, any other relevant deliverables, your experience in this lab and the definition of \(P(s),\) \textbf{justify} why you would \textbf{not} introduce an integrator term when you know for a fact that there are no step input disturbances.
      \label{lab4:report:q4}
    }
    \item{%
      Suppose your plant \(P(s)\) was, instead,
      \[
        P(s) = \frac{s + 1}{s^2 + 2 s + 3}
      \]
      and that \(d(t) = 0.\)
      Could you achieve perfect step tracking when \(K_i = 0\)?
    ...
    \item{%
      ...
      If no change improved the characteristics, explain what changes you tried, why you tried them, and why you think they did not work.
...
Lab Subtotal&                       & 30      \\ \hhline{=|=|=}
        ...
        & \ref{lab4:report}~\ref{lab4:report:q4}  & 2       \\ \hline
        & \ref{lab4:report}~\ref{lab4:report:q5}  & 2      \\ \hline
        & \ref{lab4:report}~\ref{lab4:report:q6}  & 6      \\ \hline
